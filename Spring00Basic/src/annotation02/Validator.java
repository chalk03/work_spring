			...
			if (field.getAnnotation(Validation.class) != null) {
				...
				if (type.getName().equals("int")) {
					...
					if (anno.required() && valInt == 0) {
						pass = false;
						System.out.println(field.getName() + " 는 필수로 입력해야합니다. (" + valInt + ")");
					}
					// 8. 만약 해당 필드의 값이 max값보다 크고, min값보다 작은경우
				...
				// 5. 만약 필드의 타입이 String일 경우
					...
					// 7. 만약 required가 값이 true이고, valStr 값이 없을 경우
